...
import cmapy
def visualize_heatmap(experiments, images, attmaps, epoch, cnt, phase='train', seg=False, name=None):
    ...
    for i in range(images.shape[0]):
        ...
        if seg:
            ...
            fg = np.where(attmap > 150, 255, 0).reshape(h, w, 1)
            bg = np.where(attmap < 100, 255, 0).reshape(h, w, 1)
            mask = cv2.bitwise_or(fg, bg)
            cv2.imwrite(f'./label/gt/{name}.png', fg)
            cv2.imwrite(f'./label/mask/{name}.png', mask)
